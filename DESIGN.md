# Python 练习平台 — 项目设计文档 (v1.0)

> 目标：面向计算机入门后辈的 Python 练习/检测综合平台
> 形态：Flutter 本地桌面应用（Linux 开发 → Windows 迁移）
> 状态：**v1.0 功能收官**（可运行），下一步迁移 Windows

## 一、核心闭环
学生选题 → 写代码 → 调内置 Python 运行 → 比对输出 → 对/错反馈 + 进度记录
...
## 十一、优化清单（分阶段，待用户挑选）
### P1 体验增强（改动小、收益高）
- [ ] 测试默认定位到**下一道未做的题**（当前是固定回到第 1 题）
- [ ] 测试结束时提示**还有 N 题未做**（交卷前确认弹窗）
- [ ] 判题结果面板显示**耗时**（该题跑了几个用例、耗时多少）
- [ ] 编辑器支持显示**用例通过数**（如"3/4 用例通过"）

### P2 功能扩展
- [ ] **错题本**：做错的题单独收集，支持专项重练
- [ ] **计时器**：测试板块倒计时压力模式（更像考试）
- [ ] 题目收藏/标记"待复习"
- [ ] 进度导出（JSON/CSV 分享给老师）

### P3 深度体验
- [ ] 代码编辑器增强（语法高亮、行号、自动缩进——可引 code_field / flutter_code_editor 等现成组件，注意先查开源方案）
- [ ] 更丰富的判题反馈（显示首个用例差异详情）
- [ ] 成就/称号系统（连胜、全对勋章）
- [ ] 深色/浅色主题自定义强调色
