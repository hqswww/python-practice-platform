...
**v1.5.2** —— 版本号单一维护点在 `lib/app_version.dart`（pubspec 与安装包脚本里的
副本由测试盯着，改一处漏一处会测试失败）。
...
| 方向 | 内容 |
|------|------|
| 导航 | 底栏「设置」→「**我的**」；设置降为「我的」下的子页面；「关于」在「我的」里新增入口 |
| 数据 | 「我的」页：三门语言进度/错题/测试一屏看完 + **五张 fl_chart 图表** + 基于数据的**小结** |
| 关于 | 从弹窗升级为正页：名称/版本/更新日志（离线）/检查更新/项目主页/问题反馈/第三方与本项目许可证 |
| 许可 | 新增 `LICENSE`（MIT，署名 hqswww）；更新日志 `assets/CHANGELOG.md` 改为从 `docs/releases/` 生成 |
| 数据 | 新增**练习活动记录**（按天）；导出/导入带上它 —— 趋势图与「连续练习天数」的前提 |
| 依赖 | 引入 `fl_chart ^1.2.0`（连带 `equatable`）—— 第一个较大的第三方依赖 |
...
### 构建脚本
- **验收脚本里不要「固定 sleep 再看一眼」**：`verify_macos.sh` 检查应用存活原先写的是
  `open "$APP"; sleep 10; pgrep`，紧跟一次完整构建之后跑（机器还忙着）就会误报
  「应用没能保持运行」，连带两条日志检查被跳过 —— 实际观察到一次
  「12 通过 / 1 失败 / 1 跳过」，第二次跑又全绿。这是**定时敏感**，不是崩溃：
  改成最多轮询 30 秒等它出现，出现后再确认仍存活（语义反而更严：
  起不来算失败，起来又死掉也算失败）。
