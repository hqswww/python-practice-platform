...
**v1.3.0** —— 版本号单一维护点在 `lib/app_version.dart`（pubspec 与安装包脚本里的
副本由测试盯着，改一处漏一处会测试失败）。
...
| 方向 | 内容 |
|------|------|
| 语言 | **C 与 C++ 两个独立科目**接入，各 72 题；三门语言题库/进度/成就完全独立 |
| 分发 | **安装包**：Windows `Setup.exe`（免管理员）、macOS `dmg`（拖拽安装）；绿色版保留 |
| 上手 | **首次运行向导**：第一次打开就检查三门语言的运行环境，可一键装编译器 |
| 判题 | 运行面板按语言分「交互终端 / 编译运行」，且**能逐步喂输入**；错误提示按语言给方向 |
| 稳健 | Windows **中文用户名路径**全线防御（判题工作目录改 ASCII、编译器加 `-B`） |
...
- **状态**：代码里已写固定色，`test/interactive_terminal_widget_test.dart` 也断言了
  `Color(0xFFF0F6FC)`，但用户反馈**重启后仍随主题** → 尚未定位
