...
两门语言的 **id 段规则相同**（都按分类序号走：`NN_*` → `NN*100+1..6`），
但**分类不完全相同** —— 前 7 个同名，**08–11 换成 C++ 自己的主题**
（C 是字符串/指针/结构体/进阶，C++ 是引用/类/继承/STL）。见下表。

> ⚠️ 这里原先写着「两门语言分类同名同结构」，与下面的表格自相矛盾 ——
> 照着那句做会给 C++ 建出 `08_strings.json`。以**表格**为准。
...
**id 段规则与 C 相同**（按分类序号 `NN*100+1..6`），前 7 个分类也与 C 同名；
**08–11 换成 C++ 自己的主题**（C++ 的重点在后半段：面向对象与 STL）。

⚠️ 所以「文件名与分类名」以**本表**为准，不要照 C 的表填：
...
### ⚠️ 分类序号是**跨语言**的键（加了分类就要小心）

三门语言的分类**序号必须一一对应**（第 N 个是同一教学阶段）：测试功能的
「出题范围」按序号存（见 `lib/models/test_scope.dart`），用户选的是「第几个大类」。

- 在某一门语言里**插入**一个分类、或调整顺序 → 用户已保存的范围会静默指到别的大类上
- 加分类请**三门一起加，且加在同样位置**；真要插在中间，等于让所有人的范围设置错位

`test/bank_structure.dart` 会检查每门语言的序号恰好是 1~12 不重不漏
（但判不了「第 N 个是不是同一阶段」—— 那条只能靠这里的人为约定）。
