...
支持 **Python / C** 两门语言（顶栏切换），各 12 个分类、72 道题。
...
- **多语言**：Python 与 C 各自独立的题库、进度、成就；顶栏一键切换
...
- **判题引擎**：Python 用应用内捆绑的解释器（免装）；C 用系统编译器（clang/gcc），
  编译一次复用产物；支持运行面板（Python 是交互终端，C 是编译运行）
