...
''';
  ...
  group('C 题库内容自检', () {
    ...
    // 只能成功加载一次（见 practice_grid_test.dart 的说明），
    // 所以整个文件里只有这一个用例碰题库。
    test('每道题的参考答案都能通过它自己的全部测试用例', () async {
      if (!compilerReady) return;

      final cats = await ProblemRepository()
          .loadCategories(language: ProgrammingLanguage.c);
      expect(cats, isNotEmpty, reason: 'C 题库应有分类');
      expect(cats.every((c) => c.language == ProgrammingLanguage.c), isTrue);
      ...
      expect(checked, greaterThanOrEqualTo(6), reason: '至少应校验 6 道题');
    });
  });
